...
import DictionaryAddSourcesStep from './DictionaryAddSourcesStep.vue'
...
async function close() {
  if (!await dirtyForm.requestClose())
    return
  error.value = ''
  emit('close')
}
  ...
  <Dialog :open="open" :title="$t('dictionary.addDialogTitle')" :description="$t('dictionary.addDialogDescription')" width-class="max-w-2xl" @close="close">
    <fieldset :disabled="pendingLocalCommit" class="contents">
      <div class="space-y-5">
        ...
        <DictionaryAddSourcesStep v-else-if="step === 2" :drafts="drafts" @add-example="addExample" @remove-example="removeExample" />
    ...
    <template #footer>
      <DialogFooter>
        ...
        <Button v-if="step > 1" variant="ghost" :disabled="pendingLocalCommit" @click="back">
