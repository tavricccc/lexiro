  ...
  <section class="space-y-6 text-left">
    <div>
      ...
      <Card class="p-5">
        ...
        <TrendingUp class="h-5 w-5 text-ink-500" /><p class="mt-4 text-3xl font-black">
          {{ accuracy }}%
        </p><p class="mt-1 text-xs font-bold text-ink-500">
          {{ $t('learning.accuracy') }}
        </p>
      </Card><Card class="p-5">
        <Flame class="h-5 w-5 text-ink-500" /><p class="mt-4 text-3xl font-black">
          {{ stats.streakDays }}
        </p><p class="mt-1 text-xs font-bold text-ink-500">
          {{ $t('learning.streak') }}
        </p>
      </Card><Card class="p-5">
        <Award class="h-5 w-5 text-ink-500" /><p class="mt-4 text-3xl font-black">
          {{ stats.level }}
        </p><p class="mt-1 text-xs font-bold text-ink-500">
      ...
      <div class="flex items-center gap-2">
        <BarChart3 class="h-5 w-5" /><h2 class="font-black">
          {{ $t('stats.bySet') }}
        </h2>
    ...
    </Card><Card v-if="stats.achievements.length" class="p-6">
      <div class="flex items-center gap-2">
        <Award class="h-5 w-5" /><h2 class="font-black">
          {{ $t('learning.achievements') }}
        </h2>
      </div><div class="mt-5 grid gap-3 sm:grid-cols-3">
        <div v-for="achievement in stats.achievements" :key="achievement.id" class="rounded-2xl bg-ink-100/70 p-4 dark:bg-ink-900/70">
