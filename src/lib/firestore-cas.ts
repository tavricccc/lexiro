...
export type ConditionalWriteResult =
  { written: true } | { written: false; current: unknown | null };
...
export async function writeDocumentsIfUnchanged(
  db: Firestore,
  writes: AtomicDocumentWrite[],
): Promise<boolean> {
  const prepared = writes.map((write) => ({
    ...write,
    payload:
      write.payload === null ? null : prepareFirestoreData(write.payload),
  }));
  return withSyncTimeout(
    runTransaction(db, async (transaction) => {
      const snapshots = await Promise.all(
        prepared.map((write) =>
          write.expectedHash === undefined
            ? null
            : transaction.get(write.reference),
        ),
      );
      const matches = prepared.every((write, index) => {
        const snapshot = snapshots[index];
        if (write.expectedHash === undefined) return true;
        if (!snapshot || !write.hash) return false;
        if (write.payload === null && !snapshot.exists()) return true;
...
export async function setDocIfUnchanged(
  db: Firestore,
  reference: DocumentReference<DocumentData>,
  expectedHash: string,
  ...
  return withSyncTimeout(
    runTransaction(db, async (transaction) => {
      ...
      const current = snapshot.exists() ? snapshot.data() : null;
      if (hash(current) !== expectedHash) return { written: false, current };
...
export async function deleteDocIfUnchanged(
  db: Firestore,
  reference: DocumentReference<DocumentData>,
  expectedHash: string,
  hash: CloudHash,
  ...
  return withSyncTimeout(
    runTransaction(db, async (transaction) => {
      const snapshot = await transaction.get(reference);
      if (!snapshot.exists()) return { written: true };
      const current = snapshot.data();
      if (hash(current) !== expectedHash) return { written: false, current };
      transaction.delete(reference);
      return { written: true };
    }),
    "Firestore compare-and-set delete",
  );
}
