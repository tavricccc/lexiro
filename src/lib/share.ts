...
function optionalText(value: unknown, field: string): string {
  if (typeof value !== "string") throw new Error(`${field} 格式錯誤`);
  return value.trim();
}
...
function normalizeMemberships(
  ...
  return value.map((membership, index) => {
    ...
    const wordKey = normalizeWordKey(
      requiredText(source.wordKey, `${field}[${index}].wordKey`),
    );
    if (seen.has(wordKey))
      throw new Error(`${field}[${index}] 重複指定 wordKey`);
    seen.add(wordKey);
    const word = words[wordKey];
    if (!word) throw new Error(`${field}[${index}] 指向未知單字`);
    if (
      !Array.isArray(source.senseIds) ||
      !source.senseIds.length ||
      !source.senseIds.every(
        (senseId) => typeof senseId === "string" && senseId.trim(),
      )
    )
      throw new Error(`${field}[${index}].senseIds 至少需要一個有效 senseId`);
    const senseIds = source.senseIds.map((senseId) =>
      asSenseId(String(senseId).trim()),
    );
    if (new Set(senseIds).size !== senseIds.length)
      throw new Error(`${field}[${index}].senseIds 不可重複`);
    if (
      !senseIds.every((senseId) =>
        word.senses.some((sense) => sense.id === senseId),
      )
    )
...
function normalizeQuestionsForWords(
  value: unknown,
  words: Record<WordKey, WordEntry>,
  field: string,
): LibraryQuestion[] {
...
export function normalizeSharedSet(value: unknown): SharedSet {
  ...
  const membershipKeys = new Set(
    memberships.map((membership) => membership.wordKey),
  );
  if (words.some((word) => !membershipKeys.has(word.wordKey)))
    throw new Error("單字集包含沒有關聯的單字");
  const questions = normalizeQuestionsForWords(
    source.questions,
    wordsByKey,
    "questions",
  );
...
function normalizeFolder(value: unknown, index: number): VocabFolder {
  ...
  assertKnownKeys(
    source,
    ["id", "name", "parentId", "order", "createdAt", "updatedAt"],
    `folders[${index}]`,
  );
  const rawParentId =
    source.parentId === undefined
      ? undefined
      : requiredText(source.parentId, `folders[${index}].parentId`);
...
function normalizeSet(
  ...
  assertKnownKeys(
    source,
    ["id", "setName", "folderId", "createdAt", "updatedAt"],
    `sets[${index}]`,
  );
...
export function normalizeLibraryState(value: unknown): LibraryState {
  ...
  for (const [index, [storedWordKey, word]] of Object.entries(
    rawWords,
  ).entries()) {
    const normalized = normalizeWord(word, index);
    if (storedWordKey !== normalized.wordKey)
      throw new Error(
        `library.words.${storedWordKey} 的 key 與 wordKey 不一致`,
      );
  ...
  for (const folder of folders) {
    ...
    if (
      folders.some(
        (other) =>
          other.id !== folder.id &&
          other.parentId === folder.parentId &&
  ...
  for (const folder of folders) {
    ...
    while (current?.parentId) {
      if (visited.has(current.id)) throw new Error("資料夾階層不可形成循環");
      visited.add(current.id);
      current = folders.find((candidate) => candidate.id === current?.parentId);
    }
  }
  if (!Array.isArray(source.sets)) throw new Error("library.sets 格式錯誤");
  const sets = source.sets.map((set, index) =>
    normalizeSet(set, index, folderIds),
  );
  ...
  for (const set of sets) {
    ...
    if (setNames.has(normalizedName)) throw new Error("單字集名稱不可重複");
    setNames.add(normalizedName);
  }
  const rawMemberships = requiredObject(
    source.memberships,
    "library.memberships",
  );
  ...
  for (const [setId, valueForSet] of Object.entries(rawMemberships)) {
    ...
    memberships[setId] = normalizeMemberships(
      valueForSet,
      words,
  ...
  const referencedWordKeys = new Set(
    Object.values(memberships).flatMap((membershipsForSet) =>
      membershipsForSet.map((membership) => membership.wordKey),
    ),
  );
...
function normalizeCard(value: unknown, field: string) {
  ...
  const card = {
    ...
    scheduledDays: requiredNumber(
      source.scheduledDays,
      `${field}.scheduledDays`,
    ),
    learningSteps: requiredNumber(
      source.learningSteps,
      `${field}.learningSteps`,
    ),
  ...
  if (source.lastReview !== undefined)
    return {
      ...card,
      lastReview: requiredText(source.lastReview, `${field}.lastReview`),
    };
  return card;
}
...
export function normalizeLearningProgress(value: unknown): LearningProgress {
  ...
  for (const [senseId, card] of Object.entries(rawCards))
    cards[asSenseId(senseId)] = normalizeCard(
      card,
      `learning.cards.${senseId}`,
    );
  return {
    cards,
    updatedAt: requiredText(source.updatedAt, "learning.updatedAt"),
  };
}
...
function normalizeQuestionStats(
  ...
  for (const key of QUESTION_STAT_KEYS) {
    ...
    if (row.total || row.correct || row.retry) totals[key] = row;
  }
  return totals;
}

function normalizeQuestionStatsBySense(
  value: unknown,
  field: string,
): DashboardStats["questionStatsBySense"] {
  const source = requiredObject(value, field);
  ...
  for (const [senseId, stats] of Object.entries(source)) {
    ...
    if (Object.keys(totals).length) bySense[asSenseId(senseId)] = totals;
  }
  return bySense;
}
...
export function normalizeDashboardStats(value: unknown): DashboardStats {
  ...
  const dailyHistory = requiredObject(
    source.dailyHistory,
    "stats.dailyHistory",
  );
  const normalizedDailyHistory = Object.fromEntries(
    Object.entries(dailyHistory).map(([date, activity]) => {
      const normalized = normalizeDailyActivity(
        activity,
        `stats.dailyHistory.${date}`,
      );
  ...
  return {
    totalMemoryReviews: requiredNumber(
      source.totalMemoryReviews,
      "stats.totalMemoryReviews",
    ),
    correctMemoryReviews: requiredNumber(
      source.correctMemoryReviews,
      "stats.correctMemoryReviews",
    ),
    totalQuestionReviews: requiredNumber(
      source.totalQuestionReviews,
      "stats.totalQuestionReviews",
    ),
    correctQuestionReviews: requiredNumber(
      source.correctQuestionReviews,
    ...
    dailyQuestionGoal: requiredNumber(
      ...
      "stats.dailyQuestionGoal",
    ),
    todayMemoryReviews: requiredNumber(
      source.todayMemoryReviews,
      "stats.todayMemoryReviews",
    ),
    todayMemoryCorrectReviews: requiredNumber(
      source.todayMemoryCorrectReviews,
      "stats.todayMemoryCorrectReviews",
    ),
    todayQuestionReviews: requiredNumber(
      source.todayQuestionReviews,
      "stats.todayQuestionReviews",
    ),
    todayQuestionCorrectReviews: requiredNumber(
