...
function useEmulators(): boolean {
  return (
    process.env.NODE_ENV !== "production" && config.emulatorEnabled === "true"
  );
}
...
export function getFirebaseApp(): FirebaseApp | null {
  ...
  const hasDebugToken = Boolean(
    config.appCheckDebugToken?.trim() && process.env.NODE_ENV !== "production",
  );
    ...
    try {
      if (hasDebugToken) {
        ...
        debugGlobal.FIREBASE_APPCHECK_DEBUG_TOKEN =
          config.appCheckDebugToken === "true"
            ? true
...
export function getFirebaseAuth(): Auth | null {
  ...
  if (useEmulators() && !authEmulatorConnected) {
    try {
      ...
      console.warn(
        "[Firebase Auth] emulator connection was not enabled",
        error,
      );
    }
...
export function getFirebaseFirestore(): Firestore | null {
  ...
  if (!firestore) {
    try {
      ...
      console.warn(
        "[Firebase Firestore] persistent cache is unavailable; using memory cache",
        error,
      );
      try {
        firestore = initializeFirestore(app, {
  ...
  if (useEmulators() && !firestoreEmulatorConnected) {
    try {
      ...
      console.warn(
        "[Firebase Firestore] emulator connection was not enabled",
        error,
      );
    }
