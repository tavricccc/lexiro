    ...
    (item) =>
      ...
      item.senses.some(
        (sense) =>
          sense.pos.trim() ||
          sense.meaning.trim() ||
          sense.examples.some((example) => example.trim()),
      ),
  );
}
  ...
  return (
    ...
    items.every(
      (item) =>
        ...
        item.senses.every((sense) => sense.pos.trim() && sense.meaning.trim()),
    )
  );
}
...
function isPracticeQuestion(value: unknown): value is PracticeQuestion {
  if (
    ...
    !["standard", "fillBlank", "reading"].includes(
      String(value.questionType),
    ) ||
  ...
  return (
    ...
    Number(value.answerIndex) < value.options.length
  );
}
...
function isSessionEntry(value: unknown): value is SessionEntry {
  if (
    ...
    !isStudyWord(value.item) ||
    typeof value.originalIndex !== "number" ||
    !Number.isInteger(value.originalIndex) ||
    value.originalIndex < 0
  )
    return false;
  const hasValidReadingPack =
    !(
      isPracticeQuestion(value.question) &&
      value.question.questionType === "reading"
    ) ||
    (typeof value.readingPassage === "string" &&
      value.readingPassage.trim().length > 0 &&
      typeof value.readingPackId === "string" &&
      value.readingPackId.trim().length > 0);
  return (
    (value.question === undefined || isPracticeQuestion(value.question)) &&
    (value.readingPassage === undefined ||
      typeof value.readingPassage === "string") &&
    (value.readingPackId === undefined ||
      typeof value.readingPackId === "string") &&
    hasValidReadingPack
  );
}
...
function isDraft(value: unknown): boolean {
  ...
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, ["selectedIndex", "answered"]) ||
    typeof value.answered !== "boolean"
  )
    return false;
  return (
    value.selectedIndex === null ||
    (Number.isInteger(value.selectedIndex) && Number(value.selectedIndex) >= 0)
  );
}
...
export function normalizeSession(
  ...
  if (
    ...
    return null;
  if (
    typeof source.sourceSetId !== "string" ||
    !validSetIds.has(source.sourceSetId)
  )
    return null;
  if (
    typeof source.mode !== "string" ||
    !["quiz", "fillBlank", "reading"].includes(source.mode)
  )
    return null;
  if (
    !Array.isArray(source.entries) ||
    !source.entries.length ||
    !source.entries.every(isSessionEntry)
  )
    return null;
  ...
  if (
    ...
    source.index < 0 ||
    source.index >= source.entries.length ||
    typeof source.correctCount !== "number" ||
    !Number.isInteger(source.correctCount) ||
    source.correctCount < 0 ||
    source.correctCount > source.entries.length
  )
    return null;
  if (
    !Array.isArray(source.markedForReview) ||
    source.markedForReview.length !== source.entries.length ||
    !source.markedForReview.every((value) => typeof value === "boolean")
  )
    return null;
  if (
    !Array.isArray(source.wrongEntries) ||
    !source.wrongEntries.every(isSessionEntry) ||
    !Array.isArray(source.answers) ||
    !source.answers.every(isQuizRecord) ||
    !Array.isArray(source.drafts) ||
    !source.drafts.every(isDraft) ||
    typeof source.review !== "boolean" ||
    (source.status !== "in-progress" && source.status !== "completed")
  )
    return null;
  ...
  return {
    ...
    status:
      source.status === "completed" || view === "result"
        ? "completed"
        : "in-progress",
  };
}
...
export function createEditorItem(
  item?: Pick<EditorItem, "word" | "senses"> | StudyWord | null,
): EditorItem {
  const senses =
    item && "senses" in item
  ...
  return {
    id:
      item && "id" in item && typeof item.id === "string"
        ? item.id
        : generateEditorId(),
    word: item?.word ?? "",
    senses,
