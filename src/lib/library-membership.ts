...
export function sanitizeMemberships(
  ...
  for (const membership of memberships) {
    ...
    if (
      ...
      continue;
    const senseIds = Array.from(new Set(membership.senseIds)).filter(
      (senseId) => word.senses.some((sense) => sense.id === senseId),
    );
    if (!senseIds.length) continue;
    ...
    byWord.set(
      ...
      existing
        ? {
            wordKey,
            senseIds: Array.from(new Set([...existing.senseIds, ...senseIds])),
          }
        : { wordKey, senseIds },
    );
  }
...
export function membershipsCoverWords(
  words: WordEntry[],
  memberships: SetMembership[],
): boolean {
  const wordKeys = new Set(words.map((word) => word.wordKey));
  const membershipWordKeys = new Set(
    memberships.map((membership) => membership.wordKey),
  );
  return Array.from(wordKeys).every((wordKey) =>
    membershipWordKeys.has(wordKey),
  );
}
