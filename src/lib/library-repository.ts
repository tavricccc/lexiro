...
type LibraryRecordValue = VocabFolder | LibrarySet | SetMembership[] | WordEntry | LibraryQuestion
...
export function emptyLibraryState(now = new Date().toISOString()): LibraryState {
...
function normalizeState(value: LibraryState): LibraryState {
  const folders = value.folders.some(folder => folder.id === UNCATEGORIZED_FOLDER_ID)
    ? value.folders
    : [createUncategorizedFolder(), ...value.folders]
  const normalized = normalizeLibraryState({ ...cloneJson(value), folders: cloneJson(sortFolders(folders)) })
  return { ...normalized, folders: sortFolders(normalized.folders) }
}

function recordsOf(state: LibraryState): LibraryRecord[] {
  return [
    ...state.folders.map(value => ({ kind: 'folder' as const, id: value.id, value })),
    ...state.sets.map(value => ({ kind: 'set' as const, id: value.id, value })),
    ...Object.entries(state.memberships).map(([id, value]) => ({ kind: 'membership' as const, id, value })),
    ...Object.entries(state.words).map(([id, value]) => ({ kind: 'word' as const, id, value })),
    ...state.questions.map(value => ({ kind: 'question' as const, id: value.id, value })),
  ]
}
...
function isManifest(value: unknown, generation: string): value is LibraryManifest {
  if (!value || typeof value !== 'object')
    return false
  const manifest = value as Partial<LibraryManifest>
  return manifest.schemaVersion === LIBRARY_REPOSITORY_SCHEMA_VERSION
...
export class LibraryRepository {
  ...
  private async readManifest(generation: string): Promise<LibraryManifest | null> {
  ...
  private async readBlob(hash: string): Promise<LibraryRecordValue> {
    ...
    if (value === undefined)
      throw new Error(`Library blob not found: ${hash}`)
  ...
  private async assemble(manifest: LibraryManifest): Promise<LibraryState> {
    ...
    for (const kind of RECORD_KINDS) {
      for (const [id, hash] of Object.entries(manifest.entries[kind])) {
        ...
        if (kind === 'folder')
          folders.push(value as VocabFolder)
        else if (kind === 'set')
          sets.push(value as LibrarySet)
  ...
  private async recover(): Promise<LibraryManifest | null> {
    ...
    for (const key of await keys()) {
      if (typeof key !== 'string' || !key.startsWith(manifestPrefix))
        continue
      const manifest = await this.readManifest(key.slice(manifestPrefix.length))
      if (manifest)
        candidates.push(manifest)
    }
    candidates.sort((left, right) => left.sequence - right.sequence || left.generation.localeCompare(right.generation))
  ...
  private async loadManifest(): Promise<LibraryManifest | null> {
    ...
    if (head?.schemaVersion === LIBRARY_REPOSITORY_SCHEMA_VERSION && head.generation) {
  ...
  async commit(state: LibraryState): Promise<LibraryCommitStats> {
    const run = this.commitQueue.catch(() => undefined).then(() => this.commitInternal(state))
    this.commitQueue = run
    return run
  }

  private async commitInternal(state: LibraryState): Promise<LibraryCommitStats> {
    const normalized = normalizeState(state)
    const previousManifest = await this.loadManifest()
    const knownHashes = new Set(previousManifest
      ? RECORD_KINDS.flatMap(kind => Object.values(previousManifest.entries[kind]))
      : [])
    ...
    await setMany([
      ...Array.from(pendingBlobs, ([hash, value]): [string, LibraryRecordValue] => [this.blobKey(hash), value]),
    ...
    for (const hash of pendingBlobs.keys())
    ...
    const head: LibraryHead = {
      schemaVersion: LIBRARY_REPOSITORY_SCHEMA_VERSION,
      generation,
      ...(previousManifest ? { previousGeneration: previousManifest.generation } : {}),
      updatedAt: normalized.updatedAt,
      manifestChecksum: manifestChecksum(manifest),
    }
    await set(this.headKey(), head)
    ...
    return {
      ...
      totalRecords: RECORD_KINDS.reduce((total, kind) => total + Object.keys(entries[kind]).length, 0),
      collectedKeys,
    }
  }

  /** Drops manifests and blobs that neither live generation references. */
  private async collect(current: LibraryManifest, previous: LibraryManifest | null): Promise<number> {
    const live = previous ? [current, previous] : [current]
    const liveGenerations = new Set(live.map(manifest => manifest.generation))
    const liveHashes = new Set(live.flatMap(manifest => RECORD_KINDS.flatMap(kind => Object.values(manifest.entries[kind]))))
    const manifestPrefix = `${this.prefix}:manifest:`
    ...
    for (const key of await keys()) {
      if (typeof key !== 'string')
        continue
      if (key.startsWith(manifestPrefix) && !liveGenerations.has(key.slice(manifestPrefix.length)))
        stale.push(key)
      else if (key.startsWith(blobPrefix) && !liveHashes.has(key.slice(blobPrefix.length)))
        stale.push(key)
    }
    if (stale.length)
      await delMany(stale)
  ...
  async clear(): Promise<void> {
    const owned = (await keys()).filter((key): key is string => typeof key === 'string' && key.startsWith(`${this.prefix}:`))
    if (owned.length)
      await delMany(owned)
