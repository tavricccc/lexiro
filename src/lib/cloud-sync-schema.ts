 ...
 */
export function buildV5LibraryChunks(
  uid: string,
  library: LibraryState,
): FirestoreLibraryV5Chunk[] {
  const sections: { section: LibrarySection; items: unknown[] }[] = [
    ...
    {
      section: "memberships",
      items: Object.entries(library.memberships).map(([setId, members]) => ({
        setId,
        members,
      })),
    },
  ...
  for (const { section, items } of sections) {
    const overheadBytes = estimateJsonBytes(
      v5ChunkForSection(uid, library, section, []),
    );
    let current: unknown[] = [];
    let currentBytes = 0;
    for (const item of items) {
      ...
      if (
        !current.length &&
        overheadBytes + addedBytes > MAX_LIBRARY_CHUNK_BYTES
      )
        throw new CloudSyncError(
          "cloud/data-invalid",
          `Cloud ${section} 單筆資料超過大小限制`,
        );
      if (
        current.length &&
        overheadBytes + currentBytes + addedBytes > MAX_LIBRARY_CHUNK_BYTES
      ) {
...
function manifestPartFixedBytes(
  uid: string,
  updatedAt: string,
  index: number,
): number {
...
function manifestEntryBytes(chunkId: string, checksum: string): number {
  return jsonEncoder.encode(
    `${JSON.stringify(chunkId)}:${JSON.stringify(checksum)}`,
  ).byteLength;
}

export function buildV5LibraryManifestDocuments(
  uid: string,
  chunks: FirestoreLibraryV5Chunk[],
  updatedAt: string,
): V5LibraryManifestDocuments {
  const checksums = Object.fromEntries(
    chunks.map((chunk) => [chunk.chunkId, chunk.checksum]),
  );
  ...
  for (const [chunkId, checksum] of Object.entries(checksums)) {
    ...
    if (
      currentCount > 0 &&
      manifestPartFixedBytes(uid, updatedAt, parts.length) + candidateBytes >
        MAX_LIBRARY_MANIFEST_BYTES
    ) {
    ...
    const nextBytes =
      currentBytes === 2
        ...
        : currentBytes + 1 + entryBytes;
    if (
      manifestPartFixedBytes(uid, updatedAt, parts.length) + nextBytes >
      MAX_LIBRARY_MANIFEST_BYTES
    )
      throw new CloudSyncError(
        "cloud/data-invalid",
        "Cloud library manifest part 單一項目超過大小限制",
      );
  ...
  const manifestParts = Object.fromEntries(
    parts.map((part) => [part.partId, part.checksum]),
  );
  ...
  if (estimateJsonBytes(manifest) > MAX_LIBRARY_MANIFEST_BYTES)
    throw new CloudSyncError(
      "cloud/data-invalid",
      "Cloud library manifest 分片索引仍然過大",
    );
  return { manifest, parts };
}

export function validateV5LibraryManifest(
  value: unknown,
  uid: string,
): FirestoreLibraryV5Manifest {
...
export function validateV5LibraryChunk(
  value: unknown,
  uid: string,
  documentId: string,
): FirestoreLibraryV5Chunk {
...
function validateLibraryManifestShape(
  value: unknown,
  uid: string,
): FirestoreLibraryV5Manifest {
  ...
  const hasValidChunks = Boolean(
    rawChunks &&
    typeof rawChunks === "object" &&
    !Array.isArray(rawChunks) &&
    Object.entries(rawChunks).every(
      ([chunkId, checksum]) =>
        chunkId !== "manifest" && typeof checksum === "string",
    ),
  );
  const hasValidManifestParts = Boolean(
    rawManifestParts &&
    typeof rawManifestParts === "object" &&
    !Array.isArray(rawManifestParts) &&
    Object.entries(rawManifestParts).every(
      ([partId, checksum]) =>
        partId.startsWith("manifest-part-") && typeof checksum === "string",
    ),
  );
  const hasInlineChunks =
    hasValidChunks &&
    Object.keys(rawChunks as Record<string, unknown>).length > 0;
  const hasManifestParts =
    hasValidManifestParts &&
    Object.keys(rawManifestParts as Record<string, unknown>).length > 0;
  if (
    ...
    throw new CloudSyncError(
      ...
      "Cloud library manifest schema 不受支援",
    );
  }
  const manifest = source as unknown as FirestoreLibraryV5Manifest;
  const v5Manifest = manifest;
  const expectedRevision =
    v5Manifest.manifestParts && Object.keys(v5Manifest.manifestParts).length
      ? canonicalHash({
          manifestParts: v5Manifest.manifestParts,
          updatedAt: manifest.updatedAt,
        })
      : canonicalHash({
          checksums: manifest.chunks,
          updatedAt: manifest.updatedAt,
        });
  if (expectedRevision !== manifest.revision)
    throw new CloudSyncError(
      "cloud/checksum-mismatch",
      "Cloud library manifest checksum 不一致",
    );
  return manifest;
}

export function validateV5LibraryManifestPart(
  value: unknown,
  uid: string,
  documentId: string,
): FirestoreLibraryManifestPart {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new CloudSyncError(
      "cloud/data-invalid",
      "Cloud library manifest part 格式錯誤",
    );
  const source = value as Record<string, unknown>;
  ...
  if (
    ...
    !Object.entries(chunks).every(
      ([chunkId, checksum]) =>
        chunkId.startsWith("chunk-") && typeof checksum === "string",
    )
  ) {
    throw new CloudSyncError(
      "cloud/schema-unsupported",
      "Cloud library manifest part schema 不受支援",
    );
  }
  const base = {
  ...
  if (canonicalHash(base) !== source.checksum)
    throw new CloudSyncError(
      "cloud/checksum-mismatch",
      "Cloud library manifest part checksum 不一致",
    );
  return source as unknown as FirestoreLibraryManifestPart;
}

export function validateCloudSyncHead(
  value: unknown,
  uid: string,
): FirestoreSyncHeadDoc {
  if (!value || typeof value !== "object" || Array.isArray(value))
  ...
  if (
    ...
    source.schemaVersion !== CLOUD_SCHEMA_VERSION ||
    typeof source.updatedAt !== "string" ||
    !["libraryRevision", "progressHash", "statsHash", "settingsHash"].every(
      (key) => typeof source[key] === "string",
    )
  ) {
    throw new CloudSyncError(
      "cloud/schema-unsupported",
      "Cloud sync head schema 不受支援",
    );
  }
...
function validateV5LibraryChunkSet(chunks: FirestoreLibraryV5Chunk[]): void {
  ...
  for (const section of LIBRARY_SECTIONS) {
    ...
    if (!sectionChunks.length)
      throw new CloudSyncError(
        "cloud/data-invalid",
        `Cloud library 缺少 ${section} chunk`,
      );
    if (sectionChunks.some((chunk) => !chunk.chunkId.startsWith("chunk-")))
      throw new CloudSyncError(
        "cloud/data-invalid",
        "Cloud library v5 chunk ID 格式錯誤",
      );
  }
}

export function combineV5LibraryChunks(
  chunks: FirestoreLibraryV5Chunk[],
  ...
  for (const chunk of chunks) {
    if (chunk.section === "words") {
      for (const word of chunk.items as WordEntry[]) words[word.wordKey] = word;
    } else if (chunk.section === "sets") {
      sets.push(...(chunk.items as LibrarySet[]));
    } else if (chunk.section === "memberships") {
      for (const entry of chunk.items as {
        setId: string;
...
function validateLibraryChunkShape(
  value: unknown,
  uid: string,
  documentId: string,
): FirestoreLibraryV5Chunk {
  ...
  if (
    ...
    source.ownerId !== uid ||
    source.schemaVersion !== CLOUD_SCHEMA_VERSION ||
    source.chunkId !== documentId ||
    typeof source.chunkId !== "string" ||
    typeof source.updatedAt !== "string" ||
    typeof source.checksum !== "string" ||
    !["words", "sets", "memberships", "folders", "questions"].includes(
    ...
    throw new CloudSyncError(
      "cloud/schema-unsupported",
      "Cloud library chunk schema 不受支援",
    );
  const base = {
    ownerId: source.ownerId,
    schemaVersion: source.schemaVersion,
  ...
  if (canonicalHash(integrityBase) !== source.checksum)
    throw new CloudSyncError(
      "cloud/checksum-mismatch",
      "Cloud library chunk checksum 不一致",
    );
