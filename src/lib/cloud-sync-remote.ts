...
import type {
  ...
  FirestoreLibraryManifestPart,
  FirestoreLibraryV5Chunk,
  FirestoreLibraryV5Manifest,
  FirestoreProgressDoc,
  FirestoreStatsDoc,
  FirestoreSyncHeadDoc,
  LearningProgress,
  LibraryState,
} from "@/types";
import {
...
import {
  CLOUD_LIBRARY_BATCH_SIZE,
  CLOUD_LIBRARY_DOWNLOAD_CONCURRENCY,
  CLOUD_LIBRARY_UPLOAD_CONCURRENCY,
...
import {
  buildV5LibraryChunks,
  buildV5LibraryManifestDocuments,
  combineV5LibraryChunks,
  normalizeCloudAiSettings,
  normalizeCloudProgress,
  normalizeCloudStats,
  validateCloudSyncHead,
  validateV5LibraryChunk,
  validateV5LibraryManifest,
  validateV5LibraryManifestPart,
...
export function emptyCloudLibrary(
...
export function emptyCloudProgress(
  now = new Date().toISOString(),
): LearningProgress {
...
export function emptyCloudSyncHead(
  now = new Date().toISOString(),
): FirestoreSyncHeadDoc {
  return {
    ownerId: "",
...
export async function readCloudSyncHead(
  db: Firestore,
  uid: string,
): Promise<FirestoreSyncHeadDoc | null> {
...
export async function updateCloudSyncHead(
  ...
  patch: Partial<
    Pick<
      FirestoreSyncHeadDoc,
      "libraryRevision" | "progressHash" | "statsHash" | "settingsHash"
    >
  >,
): Promise<FirestoreSyncHeadDoc> {
  const reference = cloudDocument(db, uid, "sync", "head");
  return withSyncTimeout(
    runTransaction(db, async (transaction) => {
      const snapshot = await transaction.get(reference);
      const current = snapshot.exists()
        ? validateCloudSyncHead(snapshot.data(), uid)
        : { ...emptyCloudSyncHead(), ownerId: uid };
...
export function parseCloudLibrarySnapshot(
  snapshot: QuerySnapshot<DocumentData>,
  uid: string,
): { library: LibraryState; hashes: Map<string, string>; revision: string } {
  ...
  if (!manifestDocument)
    throw new CloudSyncError(
      "cloud/data-invalid",
      "Cloud library 缺少 manifest",
    );
  return parseCloudV5Documents(snapshot.docs, uid);
}

function parseCloudV5Documents(
  ...
  for (const partId of Object.keys(manifest.manifestParts ?? {})) {
    ...
    if (!partDocument)
      throw new CloudSyncError(
        "cloud/data-invalid",
        "Cloud library manifest 缺少 manifest part",
      );
    const part = validateV5LibraryManifestPart(
      partDocument.data(),
      uid,
      partId,
    );
    if (
      manifest.manifestParts?.[partId] !== part.checksum ||
      part.updatedAt !== manifest.updatedAt
    )
      throw new CloudSyncError(
        "cloud/checksum-mismatch",
        "Cloud library manifest part 不屬於目前 manifest",
      );
  ...
  for (const part of manifestParts.values()) {
    for (const [chunkId, checksum] of Object.entries(part.chunks)) {
      if (resolvedChunks[chunkId] !== undefined)
        throw new CloudSyncError(
          "cloud/data-invalid",
          "Cloud library manifest part 包含重複 chunk",
        );
      resolvedChunks[chunkId] = checksum;
    }
  }
  const chunkChecksums = Object.keys(manifest.manifestParts ?? {}).length
    ? resolvedChunks
    : manifest.chunks;
  ...
  const chunkDocuments = documents.filter(
    (item) =>
      item.id !== "manifest" &&
      !manifestParts.has(item.id) &&
      item.id in chunkChecksums,
  );
  if (
    chunkDocuments.length !== Object.keys(chunkChecksums).length ||
    chunkDocuments.some((item) => !(item.id in chunkChecksums))
  )
    throw new CloudSyncError(
      "cloud/data-invalid",
      "Cloud library manifest 與 chunks 不一致",
    );
  const chunks = chunkDocuments.map((item) =>
    validateV5LibraryChunk(item.data(), uid, item.id),
  );
  if (chunks.some((chunk) => chunkChecksums[chunk.chunkId] !== chunk.checksum))
    throw new CloudSyncError(
      "cloud/checksum-mismatch",
      "Cloud library v5 chunk 不屬於目前 manifest",
    );
  return {
    library: combineV5LibraryChunks(chunks, manifest.updatedAt),
...
export interface CloudLibraryReadOptions {
  ...
  ) => Promise<ReadonlyMap<string, FirestoreLibraryV5Chunk>>;
  cachedRevision?: string;
  cachedHashes?: ReadonlyMap<string, string>;
  manifestData?: unknown;
}

async function resolveV5Manifest(
  db: Firestore,
  uid: string,
  ...
  const parts = await mapWithConcurrency(
    Object.keys(manifest.manifestParts),
    CLOUD_LIBRARY_DOWNLOAD_CONCURRENCY,
    async (partId) => {
      const snapshot = await withSyncTimeout(
...
async function loadExistingChunksForManifest(
  options: CloudLibraryReadOptions,
  checksums: Record<string, string>,
): Promise<ReadonlyMap<string, FirestoreLibraryV5Chunk>> {
  const loaded = options.loadExistingChunks
    ? await options.loadExistingChunks(
        Object.keys(checksums),
        new Map(Object.entries(checksums)),
      )
...
async function acquireLibraryWriteLease(
  db: Firestore,
  uid: string,
): Promise<LibraryWriteLease> {
  const token = randomUUID();
  const reference = cloudDocument(
    db,
    uid,
    "libraryLocks",
    LIBRARY_WRITE_LOCK_ID,
  );
  ...
  await withSyncTimeout(
    runTransaction(db, async (transaction) => {
      ...
      const expiresAt =
        current && typeof current.expiresAt === "number"
          ? current.expiresAt
          : 0;
...
export async function readCloudLibraryV5(
  ...
  for (
    let offset = 0;
    offset < ids.length;
    offset += CLOUD_LIBRARY_BATCH_SIZE
  ) {
    ...
    const newChunks = resolvedBatch
      .filter(({ isNew }) => isNew)
      .map(({ chunk }) => chunk);
    const activeRequests = Math.min(
      CLOUD_LIBRARY_DOWNLOAD_CONCURRENCY,
      newChunks.length,
    );
    ...
    const progress = {
      currentBatch,
      totalBatches,
      completed: Math.min(offset + batchIds.length, ids.length),
      total: ids.length,
      activeRequests: 0,
    };
...
async function readV5Chunk(
  db: Firestore,
  uid: string,
  id: string,
): Promise<FirestoreLibraryV5Chunk> {
  const snapshot = await withSyncTimeout(
...
export async function writeCloudLibraryChunksV5(
  ...
  const manifestDocuments = buildV5LibraryManifestDocuments(
    uid,
    chunks,
    library.updatedAt,
  );
  ...
  const changedChunks = chunks.filter(
    (chunk) => knownHashes.get(chunk.chunkId) !== chunk.checksum,
  );
  const totalBatches = Math.max(
    1,
    Math.ceil(chunks.length / CLOUD_LIBRARY_BATCH_SIZE),
  );
  onProgress?.({
    currentBatch: 0,
    totalBatches,
  ...
  try {
    const changedBatches = Array.from(
      { length: Math.ceil(changedChunks.length / CLOUD_LIBRARY_BATCH_SIZE) },
      (_, index) =>
        changedChunks.slice(
          index * CLOUD_LIBRARY_BATCH_SIZE,
          (index + 1) * CLOUD_LIBRARY_BATCH_SIZE,
        ),
    );
    let completedChunks = 0;
    for (
      let offset = 0;
      offset < changedBatches.length;
      offset += CLOUD_LIBRARY_UPLOAD_CONCURRENCY
    ) {
      await lease.refresh();
      const wave = changedBatches.slice(
        offset,
        offset + CLOUD_LIBRARY_UPLOAD_CONCURRENCY,
      );
      onProgress?.({
        currentBatch: Math.ceil(completedChunks / CLOUD_LIBRARY_BATCH_SIZE),
        totalBatches,
        completed: completedChunks,
        total: chunks.length,
        activeRequests: wave.length,
      });
      await Promise.all(
        wave.map(async (batch, waveIndex) => {
          ...
          for (const chunk of batch)
            writeBatchOperation.set(
              cloudDocument(db, uid, "library", chunk.chunkId),
              prepareFirestoreData(chunk),
            );
          await withSyncTimeout(
            writeBatchOperation.commit(),
            `Library upload batch ${currentBatch}`,
          );
        }),
      );
    ...
    const partBatches = Array.from(
      {
        length: Math.ceil(
          manifestDocuments.parts.length / CLOUD_LIBRARY_BATCH_SIZE,
        ),
      },
      (_, index) =>
        manifestDocuments.parts.slice(
          index * CLOUD_LIBRARY_BATCH_SIZE,
          (index + 1) * CLOUD_LIBRARY_BATCH_SIZE,
        ),
    );
    for (
      let offset = 0;
      offset < partBatches.length;
      offset += CLOUD_LIBRARY_UPLOAD_CONCURRENCY
    ) {
      await lease.refresh();
      const wave = partBatches.slice(
        offset,
        offset + CLOUD_LIBRARY_UPLOAD_CONCURRENCY,
      );
      await Promise.all(
        wave.map(async (batch, waveIndex) => {
          ...
          for (const part of batch)
            writeBatchOperation.set(
              cloudDocument(db, uid, "library", part.partId),
              prepareFirestoreData(part),
            );
          await withSyncTimeout(
            writeBatchOperation.commit(),
            `Library manifest part publish batch ${offset + waveIndex + 1}`,
          );
        }),
      );
    }
    ...
    const result = await setDocIfUnchanged(
      ...
      (value) => {
        ...
        if (source.schemaVersion !== CLOUD_SCHEMA_VERSION) return "#invalid";
    ...
    const knownStaleChunkIds = Array.from(knownHashes.keys()).filter(
      (id) => id.startsWith("chunk-") && !liveIds.has(id),
    );
...
async function scheduleCloudLibraryCleanup(
  ...
  try {
    ...
    await cleanupUnreferencedV5Chunks(
      db,
      uid,
      liveIds,
      liveManifestPartIds,
...
async function cleanupUnreferencedV5Chunks(
  ...
  try {
    const knownStaleIds = Array.from(knownHashes.keys()).filter(
      (id) => id.startsWith("chunk-") && !liveIds.has(id),
    );
    ...
    if (knownHashes.size) {
      for (
        let offset = 0;
        offset < knownStaleIds.length;
        offset += CLOUD_LIBRARY_BATCH_SIZE
      ) {
        await refreshLease();
        const batch = writeBatch(db);
        for (const staleId of knownStaleIds.slice(
          offset,
          offset + CLOUD_LIBRARY_BATCH_SIZE,
        ))
    ...
    do {
      const pageQuery = cursor
        ? query(
            ...
            orderBy(documentId()),
            startAfter(cursor),
            limit(CLOUD_LIBRARY_BATCH_SIZE),
          )
        : query(
            libraryReference,
            orderBy(documentId()),
      ...
      for (const document of snapshot.docs) {
        if (
          (document.id.startsWith("chunk-") && !liveIds.has(document.id)) ||
          (document.id.startsWith("manifest-part-") &&
            !liveManifestPartIds.has(document.id))
        ) {
          scannedStaleIds.push(document.id);
        }
      }
      cursor =
        snapshot.docs.length === CLOUD_LIBRARY_BATCH_SIZE
          ? (snapshot.docs.at(-1) ?? null)
          : null;
    } while (cursor);
    const staleIds = Array.from(
      new Set([...knownStaleIds, ...scannedStaleIds]),
    );
    for (
      let offset = 0;
      ...
      await refreshLease();
      const batch = writeBatch(db);
      for (const staleId of staleIds.slice(
        offset,
        offset + CLOUD_LIBRARY_BATCH_SIZE,
      ))
        batch.delete(cloudDocument(db, uid, "library", staleId));
...
export async function writeCloudLearningState(
    ...
    progressChanged
      ? setDocIfUnchanged(
          ...
          (value) =>
            value === null
              ? ""
              : canonicalHash(normalizeCloudProgress(value, uid)),
        )
    ...
    statsChanged
      ? setDocIfUnchanged(
          ...
          (value) =>
            value === null
              ? ""
              : canonicalHash(normalizeCloudStats(value, uid)),
        )
      : Promise.resolve({ written: true } satisfies ConditionalWriteResult),
  ]);
  ...
  if (
    progressResult.written &&
    statsResult.written &&
    (progressChanged || statsChanged)
  )
...
export async function writeCloudAiSettings(
  ...
  const result = await setDocIfUnchanged(
    ...
    payload,
    (value) =>
      value === null ? "" : canonicalHash(normalizeCloudAiSettings(value, uid)),
  );
  onProgress?.(1, 1);
  if (result.written)
