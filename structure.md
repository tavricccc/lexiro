...
- `components/library/set-tools.tsx`, `set-tool-page.tsx`, `set-word-addition.tsx`, `set-sense-supplement.tsx`, `set-metadata.tsx` — the saved set links to separate manual-add, AI-add, supplement and metadata routes instead of expanding tools beneath its word list. `set-editor.tsx` only creates new sets; the old edit URL still redirects to the set view.
