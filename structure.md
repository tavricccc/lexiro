...
- `components/ui/list.tsx` — the grouped list and every row it is made of. `ListCheckRow` is the one multi-select row: the checkbox sits where `ListChoiceRow`'s check sits, and the selected tint is painted by `.t-row::before`, which bleeds past the card's gutter so it reaches the rounded corners. `ListPicker` is the trigger itself rather than a bordered control inside a row, so a list never contains a second card.
...
- `components/library/set-tools.tsx`, `set-tool-page.tsx`, `set-word-addition.tsx`, `set-metadata.tsx` — the saved set links to separate manual-add, AI-add and metadata routes instead of expanding tools beneath its word list. `set-editor.tsx` only creates new sets; the old edit URL still redirects to the set view.
...
- `components/ui/list.tsx` — the grouped list: section, reporting row, navigation row, checkmark choice, in-place picker, switch, stepper, inline input and action row. All row variants share the same vertical padding and minimum height; every settings-shaped screen and every choice between options is built from it.
