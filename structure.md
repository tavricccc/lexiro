...
- `components/me/admin-panel.tsx`, `admin-accounts.tsx`, `admin-usage.tsx`, `admin-settings.tsx` — `/me/admin` is a three-row menu. Accounts, one account or creation, the 30-day usage report, and global plan defaults each have their own nested route; none are simulated with component-local view state.
