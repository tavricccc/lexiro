...
- `components/practice/practice-session-view.tsx` — session frame, progress and keyboard hints; `review-card.tsx` owns vocabulary/spelling feedback, `question-card.tsx` owns question answers, and both keep the advancing action at the viewport bottom. `practice-task-label.tsx` shares their task label. Spelling feedback remains visible until the learner explicitly rates it.
- `components/practice/practice-setup.tsx` — the track choice leads to one setup screen. The question count is a slider from 1 to the available questions in the chosen set, formats and difficulty remain on that screen, and the start action stays reachable. `practice-queue.ts` honors the requested count even for a passage and can include distinct questions about the same sense.
...
- `components/library/set-tools.tsx`, `set-tool-page.tsx`, `set-word-addition.tsx`, `set-sense-supplement.tsx`, `set-metadata.tsx` — the saved set puts its add and maintenance actions before the word list. The add route opens manual input immediately with an AI organization action at the top; switching to AI preserves the manual draft. Supplement and metadata remain separate tasks. `set-editor.tsx` opens the new-set form immediately and offers AI organization in the page header; the unused redirect-only edit route and duplicate AI-add route are gone.
- `components/library/set-folder-picker.tsx`, `set-move-dialog.tsx` — the one hierarchical destination list used when a set is created or moved, plus the direct move action on a saved set; nested folders keep their visible path depth and 未分類 remains an explicit destination.
...
- `tests-next/input-organizer.test.tsx` — no generation before confirmation; corrected review text proceeds to generation. Photo cases cover a single upload per ten-image batch, ordered serial batches, per-file/batch failures and cancellation retaining completed batches.
...
- `components/library/ai-set-organizer.tsx`, `input-organizer.tsx`, `word-assistant.tsx`, `lib/word-photo.ts` — AI-created sets stay on one route for all four steps: choose the destination, clean and confirm the input, then generate and edit word details. Photo capture splits selections into batches of ten; each batch uploads once and uses one multi-image provider request, with batches processed serially. It asks whether there are more and retains completed batches after cancellation or a later failure; browser image-element decoding and WebP resizing happen before upload; native WebP output is signature-checked and falls back to the bundled codec when Safari returns mislabeled bytes. Organization returns one line per word as 「英文 詞性 中文」, the part of speech inferred where the source omitted one, and `buildWordGenerationSources` in `packages/ai-contract` splits that back into the word, its `posHint` and the Chinese hint the generation step is given.
...
- `components/ui/step-actions.tsx` — the fixed safe-area action surface for guided flows and long review screens. It reserves document space while keeping the advancing action reachable without scrolling to the end; `StepFrame` uses it for every supplied footer.
