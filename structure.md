...
- `components/practice/practice-setup.tsx`, `practice-page.tsx`, `practice-queue.ts`, `use-practice-setup-choices.ts` — the track choice leads to one setup screen. The question count is a slider from 1 to the available questions in the chosen set. The one-sense-per-word switch defaults on and limits a session to one question per word; turning it off makes the full question bank available. Setup choices are saved while configuring. A saved practice session is offered for explicit resumption or deletion on reentry.
...
| Practice session snapshot      | 3       | `src/types/session.ts`: v3 stores the mixed task queue and answer choices; older snapshots are retired                                                                                                                                                                                                                                    |
