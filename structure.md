...
- `components/library/set-tools.tsx`, `set-tool-page.tsx`, `set-word-addition.tsx`, `set-sense-supplement.tsx`, `set-metadata.tsx` — the saved set has one add route that first asks between manual input and AI organization, then shows only that chosen step. Supplement and metadata remain separate tasks. `set-editor.tsx` only creates new sets; the unused redirect-only edit route and duplicate AI-add route are gone.
...
- `components/library/ai-set-organizer.tsx`, `ai-set-generation.tsx`, `input-organizer.tsx`, `lib/ai-set-draft.ts`, `lib/word-photo.ts` — AI-created sets move through two routes: choose the destination, clean and confirm the input first, then generate and edit word details. A session-only draft carries the destination through those pages, so starting inside a folder files the finished set there. Photo capture accepts up to ten images per selection, organizes them serially, asks whether there are more, and retains every completed batch; browser image-element decoding and WebP resizing happen before upload so Safari camera-library photos follow the same path. Organization returns one line per word as 「英文 詞性 中文」, the part of speech inferred where the source omitted one, and `buildWordGenerationSources` in `packages/ai-contract` splits that back into the word, its `posHint` and the Chinese hint the generation step is given.
- `components/ui/list.tsx` — the grouped list: section, reporting row, navigation row, checkmark choice, multi-select check row, in-place picker, switch, stepper, inline input and action row. All row variants share the same vertical padding and minimum height; every settings-shaped screen and every choice between options is built from it. A chosen row is marked by the same bare check whether it is one of many or several of many, and its tint is painted by `.t-row::before`, which bleeds past the card's gutter so it reaches the rounded corners. `ListPicker` is the select trigger itself rather than a bordered control inside a row, so a list never contains a second card.
...
- `tests-next/admin-usage.test.tsx` — cost per billable unit against the quoted rate, its sign, and the widest gap first.
