...
The browser sends Firebase-authenticated, data-only requests to `NEXT_PUBLIC_AI_WORKER_URL`. The private Worker owns provider credentials, prompts, output schemas, tier settings and billing. There are no browser API keys, provider settings, manual prompt panels or token-usage UI.
...
`lib/managed-client.ts` refreshes the Firebase token once on HTTP 401, rejects results after account changes, and consumes Responses text events. UI shows Lite, Thinking and Pro with point estimates. The backend independently validates requests and computes charges; client estimates are not an authorization boundary.
