...
export function SettingsPage() {
  ...
  return <div><PageHeader title={t("settings.title")} description={t("settings.description")} /><div className="grid gap-10 lg:grid-cols-2">
    <Section title={t("settings.appearance")}><Field label={t("settings.theme")}><select value={theme} onChange={(event) => setTheme(event.target.value)} className="h-11 w-full rounded-xl border bg-surface px-3"><option value="system">{t("settings.system")}</option><option value="light">{t("settings.light")}</option><option value="dark">{t("settings.dark")}</option></select></Field></Section>
    ...
    <Section title={t("settings.account")}><p className="text-sm leading-6 text-ink-muted">{cloud.configured ? (cloud.user?.email ?? t("settings.offlineReady")) : t("settings.notConfigured")}</p><div className="flex flex-wrap gap-2">{cloud.configured && !cloud.user && <Button onClick={() => { const hasGuestData = library.state.sets.length > 0 || learning.stats.totalMemoryReviews > 0 || learning.stats.totalQuestionReviews > 0; if (hasGuestData) setConfirmSignIn(true); else void cloud.signIn(); }}>{t("settings.signIn")}</Button>}{cloud.user && <><Button onClick={() => void cloud.sync()} disabled={cloud.status === "syncing"}>{t("settings.syncNow")}</Button><Button variant="secondary" onClick={() => void cloud.signOut()}>{t("settings.signOut")}</Button></>}<span className="inline-flex items-center rounded-full bg-brand-soft px-3 py-1.5 text-xs font-medium text-brand-strong">{syncStatusLabel(cloud.status)}</span>{cloud.pending && <span className="inline-flex items-center rounded-full bg-amber-50 px-3 py-1.5 text-xs font-medium text-amber-800 dark:bg-amber-950/40 dark:text-amber-200">{t("settings.syncPending")}</span>}</div>{cloud.error && <p className="text-sm text-red-700">{cloud.error}</p>}</Section>
    {message && <p role="status" className="lg:col-span-2 rounded-2xl bg-brand-soft px-5 py-4 text-sm text-brand-strong">{message}</p>}
...
function Field({ label, children }: { label: string; children: ReactNode }) { return <label><span className="mb-2 block text-xs font-semibold text-ink-muted">{label}</span>{children}</label>; }
