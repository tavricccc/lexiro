...
import type { AiProvider, AiSettings as CanonicalAiSettings, DashboardStats, LearningProgress, LibraryState } from "@/types";
...
import { downloadAiSettings, loadAiSettingsState, parseAiSettingsJson, saveAiSettings, waitForAiSettingsPersistence } from "@/src/lib/ai-provider";
...
export function SettingsPage() {
  ...
  useEffect(() => {
    void loadAiSettingsState().then((settings) => { setAi(fromCanonicalAiSettings(settings)); setApiKey(settings.apiKey); });
  }, []);
  ...
  const saveAi = async () => { const normalized = normalizeAiSettings(ai); setAi(normalized); saveAiSettings(toCanonicalAiSettings(normalized, apiKey)); await waitForAiSettingsPersistence(); setMessage(t("settings.aiSaved")); };
  const exportBackup = () => {
  ...
  const importAi = async (file: File) => {
    try { const value = parseAiSettingsJson(await file.text()); const next = { ...value, apiKey }; setAi(fromCanonicalAiSettings(next)); saveAiSettings(next); await waitForAiSettingsPersistence(); setMessage(t("settings.aiSaved")); }
    catch (reason) { setMessage(t("settings.invalidBackup", { message: reason instanceof Error ? reason.message : String(reason) })); }
  };
  ...
  return <div><PageHeader title={t("settings.title")} description={t("settings.description")} /><div className="grid gap-10 lg:grid-cols-2">
    ...
    <Section title={t("settings.ai")}><p className="text-sm leading-6 text-muted-foreground">{t("settings.aiDescription")}</p><Field label={t("settings.provider")}><select value={ai.provider} onChange={(event) => setAi({ ...ai, provider: event.target.value })} className="h-11 w-full rounded-xl border bg-card px-3">{["openai", "anthropic", "google", "custom"].map((provider) => <option key={provider} value={provider}>{provider === "custom" ? "OpenAI compatible" : provider}</option>)}</select></Field><Field label={t("settings.model")}><Input value={ai.model} onChange={(event) => setAi({ ...ai, model: event.target.value })} /></Field><Field label={t("settings.endpoint")}><Input value={ai.endpoint} onChange={(event) => setAi({ ...ai, endpoint: event.target.value })} /></Field><Field label={t("settings.batchSize")}><Input type="number" min={5} max={20} value={ai.batchSize} onChange={(event) => setAi({ ...ai, batchSize: Number(event.target.value) })} /></Field><Field label={t("settings.apiKey")}><Input type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} /></Field><div className="flex flex-wrap gap-2"><Button onClick={() => void saveAi()}>{t("settings.saveAi")}</Button><Button variant="secondary" onClick={() => downloadAiSettings(toCanonicalAiSettings(ai, apiKey))}><Download className="size-4" />{t("settings.exportAi")}</Button><Button asChild variant="ghost"><label className="cursor-pointer"><Upload className="size-4" />{t("settings.importAi")}<input type="file" accept=".json,application/json" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void importAi(file); }} /></label></Button></div></Section>
...
function normalizeAiSettings(value: unknown): AiSettings { const source = value && typeof value === "object" ? value as Partial<AiSettings> : {}; const batchSize = typeof source.batchSize === "number" && Number.isFinite(source.batchSize) ? Math.min(20, Math.max(5, Math.round(source.batchSize))) : 10; return { mode: source.mode === "api" ? "api" : "manual", provider: typeof source.provider === "string" && ["openai", "anthropic", "google", "custom"].includes(source.provider) ? source.provider : "openai", model: typeof source.model === "string" ? source.model : "gpt-5-mini", endpoint: typeof source.endpoint === "string" ? source.endpoint : "", batchSize }; }
function toCanonicalAiSettings(settings: AiSettings, apiKey: string): CanonicalAiSettings { const normalized = normalizeAiSettings(settings); return { enabled: Boolean(apiKey.trim()), provider: normalized.provider as AiProvider, apiKey, baseUrl: normalized.endpoint, model: normalized.model, batchSize: normalized.batchSize }; }
function fromCanonicalAiSettings(settings: CanonicalAiSettings): AiSettings { return { mode: settings.enabled ? "api" : "manual", provider: settings.provider, model: settings.model, endpoint: settings.baseUrl, batchSize: settings.batchSize }; }
