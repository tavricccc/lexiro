...
export function SetWordRow({ entry, setId, cards }: { entry: ViewWord; setId: string; cards: Record<SenseId, CardProgress> }) {
  return <div>
    <div className="flex items-center justify-between gap-3"><p className="text-left text-base font-medium">{entry.word}</p><Button asChild variant="ghost" size="icon" aria-label={t("setDetail.edit")}><Link href={`/sets/${setId}/words/${encodeURIComponent(entry.wordKey)}/edit`}><Icons.edit /></Link></Button></div>
