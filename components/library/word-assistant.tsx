...
export function WordAssistant({ onApply, onClose }: { onApply: (rows: AssistedWordRow[]) => void; onClose: () => void }) {
  ...
  return <section className="mt-6 rounded-2xl bg-muted p-5 sm:p-6"><div className="flex items-start gap-4"><div className="min-w-0 flex-1"><h2 className="font-semibold">{t("setEditor.aiAssist")}</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{t("setEditor.aiAssistDescription")}</p></div><Button type="button" variant="ghost" size="icon" className="size-9 min-h-9" onClick={onClose}><X className="size-4" /><span className="sr-only">{t("common.cancel")}</span></Button></div><label className="mt-5 block"><span className="mb-2 block text-xs font-semibold text-muted-foreground">{t("setEditor.rawWords")}</span><textarea value={raw} onChange={(event) => setRaw(event.target.value)} placeholder={t("setEditor.rawWordsPlaceholder")} className="min-h-32 w-full rounded-xl border bg-card px-3.5 py-3 text-sm leading-6 outline-none focus-visible:border-ring" /></label><label className="mt-3 flex items-center gap-2 text-sm"><input type="checkbox" checked={examples} onChange={(event) => setExamples(event.target.checked)} className="accent-primary" />{t("setEditor.generateExamples")}</label><div className="mt-4 flex flex-wrap gap-2"><Button type="button" disabled={!sources.length} onClick={createPrompt}>{t("setEditor.buildPrompt")}</Button><Button type="button" variant="secondary" disabled={!sources.length || busy} onClick={() => void callAi()}><Sparkles className="size-4" />{busy ? t("questions.generating") : t("setEditor.callAi")}</Button>{prompt && <Button type="button" variant="ghost" onClick={() => void copyToClipboard(prompt)}><Clipboard className="size-4" />{t("questions.copyPrompt")}</Button>}</div>{prompt && <textarea readOnly value={prompt} className="mt-4 h-36 w-full resize-y rounded-xl border bg-card p-3 text-xs leading-5" />}<label className="mt-4 block"><span className="mb-2 block text-xs font-semibold text-muted-foreground">{t("setEditor.aiResponse")}</span><textarea value={response} onChange={(event) => setResponse(event.target.value)} className="h-36 w-full resize-y rounded-xl border bg-card p-3 text-xs leading-5 outline-none focus-visible:border-ring" /></label>{error && <p role="alert" className="mt-3 text-sm text-red-700">{error}</p>}<Button type="button" className="mt-4" disabled={!response.trim()} onClick={apply}>{t("setEditor.applyPreview", { count: sources.length })}</Button></section>;
}
