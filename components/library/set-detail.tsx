...
export function SetDetail({ setId }: { setId: string }) {
  ...
  return (
    <div>
      <Link href="/library" className="mb-5 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />{t("setDetail.back")}</Link>
      ...
      {words.length === 0 ? <div className="border-y py-12 text-center text-sm text-muted-foreground">{t("setDetail.emptyWords")}</div> : (
        <div className="divide-y border-y">
          {words.map(({ word, sense }) => <article key={sense.id} className="grid gap-2 py-5 sm:grid-cols-[180px_100px_minmax(0,1fr)] sm:gap-5"><div className="text-lg font-semibold tracking-[-0.02em]">{word.word}</div><div className="text-sm font-medium text-foreground">{sense.pos}</div><div><div className="font-medium">{sense.meaningZh}</div>{sense.examples[0] && <p className="mt-2 text-sm leading-6 text-muted-foreground">{sense.examples[0]}</p>}</div></article>)}
        </div>
      )}
