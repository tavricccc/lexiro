      ...
      <section className="overflow-hidden rounded-2xl border bg-surface">
        <div className="flex min-h-14 items-center gap-1 border-b px-3 sm:px-4">
          ...
          <nav className="flex min-w-0 flex-1 items-center overflow-x-auto" aria-label={t("library.folderPath")}>
            ...
            {breadcrumbs.map((folder, index) => <span key={folder.id} className="flex shrink-0 items-center"><ChevronRight className="size-3.5 text-ink-muted" /><Breadcrumb active={index === breadcrumbs.length - 1} label={folder.name} onClick={() => openFolder(folder.id)} /></span>)}
          </nav>
        ...
        {renaming && currentFolder && <div className="border-b bg-canvas px-4 py-3"><form className="flex max-w-md gap-2" onSubmit={(event) => { event.preventDefault(); if (!renameValue.trim()) return; void renameFolder(currentFolder.id, renameValue).then(() => setRenaming(false)).catch(() => setFolderError(t("library.folderNameConflict"))); }}><Input autoFocus value={renameValue} onChange={(event) => setRenameValue(event.target.value)} /><Button type="submit" size="sm">{t("common.confirm")}</Button><Button type="button" size="sm" variant="ghost" onClick={() => setRenaming(false)}>{t("common.cancel")}</Button></form><label className="mt-3 block max-w-md"><span className="mb-1.5 block text-xs text-ink-muted">{t("library.moveFolder")}</span><select value={currentFolder.parentId ?? ""} onChange={(event) => void moveFolder(currentFolder.id, event.target.value || undefined).catch(() => setFolderError(t("library.folderNameConflict")))} className="h-10 w-full rounded-xl border bg-surface px-3 text-sm"><option value="">{t("library.rootFolder")}</option>{buildFolderOptions(state.folders).filter((folder) => folder.id !== currentFolder.id && folder.id !== UNCATEGORIZED_FOLDER_ID && !collectFolderIds(state.folders, currentFolder.id).has(folder.id)).map((folder) => <option key={folder.id} value={folder.id}>{folder.label}</option>)}</select></label>{folderError && <p className="mt-2 text-xs text-red-700" role="alert">{folderError}</p>}</div>}

        <div className="hidden grid-cols-[minmax(0,1fr)_180px_32px] border-b bg-canvas px-5 py-2 text-xs font-medium text-ink-muted sm:grid"><span>{t("library.nameColumn")}</span><span>{t("library.contentColumn")}</span><span /></div>
        ...
        {empty && <EmptyLocation searching={Boolean(query.trim())} />}
      </section>
...
function Breadcrumb({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) { return <button type="button" onClick={onClick} aria-current={active ? "page" : undefined} className={`whitespace-nowrap rounded-lg px-2.5 py-1.5 text-sm ${active ? "font-semibold text-ink" : "text-ink-muted hover:bg-ink/5 hover:text-ink"}`}>{label}</button>; }
function FolderRow({ folder, itemCount, onOpen }: { folder: VocabFolder; itemCount: number; onOpen: () => void }) { return <button type="button" onClick={onOpen} aria-label={t("library.openFolder", { name: folder.name })} className="grid w-full grid-cols-[minmax(0,1fr)_32px] items-center gap-3 border-b px-4 py-4 text-left last:border-b-0 hover:bg-canvas focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand sm:grid-cols-[minmax(0,1fr)_180px_32px] sm:px-5"><span className="flex min-w-0 items-center gap-3"><Folder className="size-5 shrink-0 fill-[var(--brand-soft)] text-brand" /><span className="truncate font-medium">{folder.name}</span></span><span className="hidden text-xs text-ink-muted sm:block">{t("library.itemCount", { count: itemCount })}</span><ChevronRight className="size-4 text-ink-muted" /></button>; }
function SetRow({ id, name, senseCount, learned, due, questionCount }: { id: string; name: string; senseCount: number; learned: number; due: number; questionCount: number }) { return <Link href={`/sets/${id}`} className="group grid grid-cols-[minmax(0,1fr)_32px] items-center gap-3 border-b px-4 py-4 last:border-b-0 hover:bg-canvas focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand sm:grid-cols-[minmax(0,1fr)_180px_32px] sm:px-5"><span className="flex min-w-0 items-center gap-3"><BookMarked className="size-5 shrink-0 text-ink-muted" /><span className="min-w-0"><span className="block truncate font-medium group-hover:text-brand-strong">{name}</span><span className="mt-1 block truncate text-xs text-ink-muted sm:hidden">{t("library.senseCount", { count: senseCount })} · {t("setDetail.dueCount", { count: due })}</span></span></span><span className="hidden text-xs leading-5 text-ink-muted sm:block">{t("library.senseCount", { count: senseCount })} · {t("setDetail.learnedCount", { count: learned })}<br />{t("setDetail.dueCount", { count: due })} · {t("library.questionCount", { count: questionCount })}</span><ChevronRight className="size-4 text-ink-muted" /></Link>; }
...
function EmptyLocation({ searching }: { searching: boolean }) { return <div className="px-6 py-14 text-center"><FolderOpen className="mx-auto size-8 text-ink-muted" /><h2 className="mt-4 text-lg font-semibold">{searching ? t("library.noResults") : t("library.emptyFolderTitle")}</h2><p className="mx-auto mt-2 max-w-md text-sm text-ink-muted">{searching ? t("library.searchHint") : t("library.emptyFolderDescription")}</p></div>; }
function Status({ text }: { text: string }) { return <div className="px-5 py-10 text-sm text-ink-muted">{text}</div>; }
