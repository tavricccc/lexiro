...
function Breadcrumb({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) { return <button type="button" onClick={onClick} aria-current={active ? "page" : undefined} className={`whitespace-nowrap rounded-lg px-2.5 py-1.5 text-sm ${active ? "font-semibold text-foreground" : "text-muted-foreground hover:bg-ink/5 hover:text-foreground"}`}>{label}</button>; }
function FolderRow({ folder, itemCount, onOpen }: { folder: VocabFolder; itemCount: number; onOpen: () => void }) { return <button type="button" onClick={onOpen} aria-label={t("library.openFolder", { name: folder.name })} className="grid w-full grid-cols-[minmax(0,1fr)_32px] items-center gap-3 border-b px-4 py-4 text-left last:border-b-0 hover:bg-[var(--surface-stage)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/40 sm:grid-cols-[minmax(0,1fr)_180px_32px] sm:px-5"><span className="flex min-w-0 items-center gap-3"><Folder className="size-5 shrink-0 fill-[var(--brand-soft)] text-brand" /><span className="truncate font-medium">{folder.name}</span></span><span className="hidden text-xs text-muted-foreground sm:block">{t("library.itemCount", { count: itemCount })}</span><ChevronRight className="size-4 text-muted-foreground" /></button>; }
function SetRow({ id, name, senseCount, learned, due, questionCount }: { id: string; name: string; senseCount: number; learned: number; due: number; questionCount: number }) { return <Link href={`/sets/${id}`} className="group grid grid-cols-[minmax(0,1fr)_32px] items-center gap-3 border-b px-4 py-4 last:border-b-0 hover:bg-[var(--surface-stage)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/40 sm:grid-cols-[minmax(0,1fr)_180px_32px] sm:px-5"><span className="flex min-w-0 items-center gap-3"><BookMarked className="size-5 shrink-0 text-muted-foreground" /><span className="min-w-0"><span className="block truncate font-medium group-hover:text-foreground">{name}</span><span className="mt-1 block truncate text-xs text-muted-foreground sm:hidden">{t("library.senseCount", { count: senseCount })} · {t("setDetail.dueCount", { count: due })}</span></span></span><span className="hidden text-xs leading-5 text-muted-foreground sm:block">{t("library.senseCount", { count: senseCount })} · {t("setDetail.learnedCount", { count: learned })}<br />{t("setDetail.dueCount", { count: due })} · {t("library.questionCount", { count: questionCount })}</span><ChevronRight className="size-4 text-muted-foreground" /></Link>; }
...
function EmptyLocation({ searching }: { searching: boolean }) { return <div className="px-6 py-14 text-center"><FolderOpen className="mx-auto size-8 text-muted-foreground" /><h2 className="mt-4 text-lg font-semibold">{searching ? t("library.noResults") : t("library.emptyFolderTitle")}</h2><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">{searching ? t("library.searchHint") : t("library.emptyFolderDescription")}</p></div>; }
