...
export function QuestionEditor({ questionId }: { questionId?: string }) {
  ...
  return <form onSubmit={submit} className="mx-auto max-w-3xl"><PageHeader title={questionId ? t("questions.edit") : t("questions.new")} actions={<Button asChild variant="ghost"><Link href="/questions"><ArrowLeft className="size-4" />{t("common.back")}</Link></Button>} />
    <div className="grid gap-5 rounded-[1.75rem] bg-brand-soft p-6 sm:grid-cols-2"><Field label={t("questions.type")}><select {...form.register("questionStyle")} className="h-11 w-full rounded-xl border bg-surface px-3"><option value="standard">{t("questions.standard")}</option><option value="fillBlank">{t("questions.fillBlank")}</option></select></Field><Field label={t("questions.linkedSense")}><select {...form.register("source", { required: true })} className="h-11 w-full rounded-xl border bg-surface px-3">{senses.map((sense) => <option key={sense.value} value={sense.value}>{sense.label}</option>)}</select></Field><Field label={t("questions.difficulty", { level: "" })}><select {...form.register("difficulty", { valueAsNumber: true })} className="h-11 w-full rounded-xl border bg-surface px-3"><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></Field></div>
    <div className="mt-7 grid gap-5"><Field label={t("questions.prompt")}><Input {...form.register("prompt", { required: true })} /></Field><div className="grid gap-4 sm:grid-cols-2">{[0,1,2,3].map((index) => <Field key={index} label={t("questions.optionLabel", { index: index + 1 })}><Input {...form.register(`option${index}` as keyof Values, { required: true })} /></Field>)}</div><Field label={t("questions.correct")}><select {...form.register("answerIndex", { valueAsNumber: true })} className="h-11 w-full rounded-xl border bg-surface px-3">{[0,1,2,3].map((index) => <option key={index} value={index}>{index + 1}</option>)}</select></Field><Field label={t("questions.explanation")}><Input {...form.register("explanation")} /></Field></div><div className="mt-7 flex justify-end"><Button type="submit">{t("questions.save")}</Button></div>
  </form>;
}

function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="mb-2 block text-xs font-semibold text-ink-muted">{label}</span>{children}</label>; }
