...
export function QuestionsPage() {
  ...
  return <div>
    ...
    <div className="flex flex-col gap-3 sm:flex-row"><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("questions.search")} /><select value={kind} onChange={(event) => setKind(event.target.value)} className="h-11 rounded-xl border bg-surface px-3 text-sm"><option value="all">{t("questions.allTypes")}</option><option value="standard">{t("questions.standard")}</option><option value="fillBlank">{t("questions.fillBlank")}</option><option value="reading">{t("questions.reading")}</option></select><select value={difficulty} onChange={(event) => setDifficulty(event.target.value)} className="h-11 rounded-xl border bg-surface px-3 text-sm"><option value="all">{t("questions.difficulty", { level: "—" })}</option>{[1,2,3].map((value) => <option key={value} value={value}>{t("questions.difficulty", { level: value })}</option>)}</select></div>
    ...
    <div className="mt-6 divide-y border-y">{questions.map((question) => {
      ...
      return <article key={question.id} className="flex items-start gap-4 py-5"><div className="min-w-0 flex-1"><div className="flex flex-wrap gap-2 text-xs text-ink-muted"><span className="font-semibold text-brand-strong">{type}</span><span>{t("questions.difficulty", { level: question.difficulty })}</span></div><Link href={question.kind === "reading" ? `/questions/reading/${question.id}/edit` : `/questions/${question.id}/edit`} className="mt-2 block font-medium leading-6 hover:text-brand-strong">{prompt}</Link></div><Button type="button" variant="ghost" size="icon" onClick={() => setDeleteTarget(question.id)} aria-label={t("questions.delete")}><Trash2 className="size-4" /></Button></article>;
    })}</div>
